...
<template>
    <div class="hidden w-64 flex-col border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-background-dark md:flex sticky top-0 h-screen flex-shrink-0">
        <!-- Brand/Logo Area -->
        ...
        <nav class="flex flex-col gap-2 p-4 flex-1 overflow-y-auto">
            <p class="px-3 pb-2 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Menu</p>
            ...
            <Link :href="route('dashboard')" 
                ...
                <span class="material-symbols-outlined text-xl" :class="{ 'fill-1': route().current('dashboard') }">bookmark</span>
            ...
            <Link :href="route('categories.index')" 
                ...
                <span class="material-symbols-outlined text-xl" :class="{ 'fill-1': route().current('categories.*') }">folder</span>
        ...
        <div class="mt-auto p-4 border-t border-slate-100 dark:border-slate-800 flex-shrink-0">
            <Link href="#" class="flex items-center gap-3 rounded-lg px-3 py-2 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                <span class="material-symbols-outlined text-xl">settings</span>
                <span class="text-sm font-medium leading-normal">Settings</span>
            </Link>
            
            <div class="flex items-center gap-3 mt-4 px-2 py-3">
                <div class="flex items-center justify-center size-10 rounded-full bg-primary/10 text-primary font-bold flex-shrink-0">
                    {{ authUser?.first_name?.charAt(0) || '' }}{{ authUser?.last_name?.charAt(0) || '' }}
                </div>
                <div class="flex flex-col overflow-hidden">
                    <span class="text-sm font-semibold text-slate-900 dark:text-white truncate">{{ authUser?.first_name }} {{ authUser?.last_name }}</span>
